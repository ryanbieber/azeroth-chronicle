...
- Optional guided voice-over is implemented for all 117 StoryNodes as repository-backed MP3 assets with opt-in persistent controls, visible transcripts, AI disclosure, deterministic Gradio generation, provenance and hash manifests, missing-asset validation, interruption behavior, and browser coverage for on/off/re-enable and chapter changes. It narrates guided-tour text only.
